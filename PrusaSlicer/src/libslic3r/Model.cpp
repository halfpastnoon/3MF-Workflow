...
void Model::convert_multipart_object(unsigned int max_extruders)
{
    ...
    //std::ifstream ext_map;
    //boost::filesystem::path full_path(boost::filesystem::current_path());
    //std::cout << full_path << std::endl;
    //read extruder map file into vector
    //FIXME copy the config etc?
    bool flag = true;
    std::string ext_map_name = object->input_file.substr(0, object->input_file.rfind("\\")+1) + "extruder_map.dat";
    std::cout << ext_map_name << std::endl;
    std::vector<int> extruder_map;
    std::ifstream exis(ext_map_name, std::ios::binary);
    if(!exis.is_open()) flag = false;
    else{
        int ext_sz;
        exis.read((char*)&ext_sz, sizeof(int));
        extruder_map.resize(ext_sz);
        exis.read((char *)&extruder_map[0], ext_sz*sizeof(int));
        exis.close();
    }
    unsigned int mat_ass = 0;
	...
	for (const ModelObject* o : this->objects)
    	for (const ModelVolume* v : o->volumes) {
            ...
            int counter = 1;
            auto copy_volume = [o, max_extruders, &counter, &extruder_counter, &mat_ass, flag, extruder_map](ModelVolume *new_v) {
                assert(new_v != nullptr);
                new_v->name = (counter > 1) ? o->name + "_" + std::to_string(counter++) : o->name;
                new_v->config.set("extruder", (flag)?extruder_map[mat_ass]:1);
